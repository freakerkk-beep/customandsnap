...
export default function ProductConfigurator({ product }: ProductConfiguratorProps) {
  ...
  const orderMockupRef = useRef<HTMLDivElement>(null);
  ...
  const handleFinish = async () => {
    ...
    setFinishing(true);
    try {
      const imageDataUrl = await makeImage(orderMockupRef.current, 2);
  ...
  return (
    <div ref={sectionRef} className="scroll-mt-20 px-4 pb-10">
      ...
      <div
        ref={orderMockupRef}
        aria-hidden="true"
        className="fixed left-[-10000px] top-0 w-[760px] bg-[#FBF6FF] px-7 py-7"
      >
        <ClickerTray
          keys={config.customData.keys}
          palette={palette}
          switchType={config.customData.switchType}
          showMeta
          product={product}
        />
      </div>
